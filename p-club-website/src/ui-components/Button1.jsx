...
const Button = ({ type = 'button', children, isSubmitting = false, className = '' }) => {
  return (
    <button
      type={type}
      className={`relative cursor-pointer py-3 px-4 text-center font-barlow inline-flex justify-center text-base uppercase text-white rounded-lg border-solid transition-transform duration-300 ease-in-out group outline-offset-4 focus:outline focus:outline-2 focus:outline-white focus:outline-offset-4 overflow-hidden ${className}`}
